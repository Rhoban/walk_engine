...
void WalkEngine::replan(double timeSinceLastStep)
{
  std::vector<rhoban_utils::PolySpline*> splines;

  splines.push_back(&supportFoot().xSpline);
  splines.push_back(&flyingFoot().xSpline);

  splines.push_back(&supportFoot().ySpline);
  splines.push_back(&flyingFoot().ySpline);

  splines.push_back(&supportFoot().yawSpline);
  splines.push_back(&flyingFoot().yawSpline);

  // For all spline, we add copy the begining and the current situation
  for (rhoban_utils::PolySpline* spline : splines)
  {
    rhoban_utils::PolySpline oldSpline = *spline;
    spline->clear();
    spline->addPoint(0, oldSpline.get(0), oldSpline.getVel(0));
    spline->addPoint(timeSinceLastStep, oldSpline.get(timeSinceLastStep), oldSpline.getVel(timeSinceLastStep));
  }

  // And happen a new end according to current orders
  planStepEnd();
}

void WalkEngine::planStepEnd()
{
  // X and Y splines
